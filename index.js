...
const getData = async (data, table) => {
    ...
    if (body.messages.length > 0) throw new Error(body.messages);
    return JSON.parse(body);
    // if (!body.messages || body.messages.lenght > 0) return JSON.parse(body);
    // console.log(body.messages)
...
try {
    ...
    const artigos = await getData("faminome, ref, design1", "st"); // Obter dados sobre os artigos & servicos
    artigos.result.forEach(element => {
